...
export default function PropertyHeader({ apiOnline, enrichStatus }) {
  ...
  return (
    <header className="shrink-0 border-b border-panel-border bg-[#060606]/95 backdrop-blur-sm">
      <div className="flex flex-wrap items-center justify-between gap-4 px-4 py-3 md:px-6">
        <div className="min-w-0">
          ...
          <p className="font-mono text-[9px] uppercase tracking-[0.2em] text-ink-muted">
            Address enrichment · Crawl4AI extraction
          </p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-x-6 gap-y-2 border-t border-panel-border px-4 py-2 md:px-6">
        <StatusChip
          label={apiOnline === false ? 'API: Offline' : apiOnline ? 'API: Online' : 'API: Checking'}
          status={apiOnline === false ? 'critical' : apiOnline ? 'stable' : 'watch'}
          pulse={apiOnline === true}
        />
        {enrichStatus ? (
          <StatusChip label={`Enrichment: ${enrichStatus}`} status="live" />
        ) : (
          <StatusChip label="Enrichment: Idle" status="stable" />
        )}
        <StatusChip label="Sources: Nominatim + optional crawl" status="stable" />
      </div>
    </header>
  )
}
